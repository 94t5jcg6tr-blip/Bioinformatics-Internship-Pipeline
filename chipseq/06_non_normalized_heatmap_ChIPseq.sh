...
# This script creates non-normalized heatmaps and average signal plots for the ChIP-seq data.
# It uses ChIP/Input ratio BigWig files without dm6 spike-in scaling to compare H3K9me3
# and H4K20me3 signal between WT and the different KO cell lines.
#
...
# WT and KO cell lines easier to compare without applying spike-in normalization.
...
# These BigWigs contain ChIP/Input ratios without dm6 spike-in scaling.
BW_DIR = "04_deeptools/bigwig_non_normalized_ratio"
PEAK_DIR = "03_macs2/peaks"
OUT_DIR = "05_analysis/02_peak_signal/non_normalized_heatmaps"

OUTPDF = (
    f"{OUT_DIR}/"
    "heatmap_spikein_ratio_union_min2_Si11Si12_merged_non_normalized.pdf"
)

OUTPNG = (
    f"{OUT_DIR}/"
    "heatmap_spikein_ratio_union_min2_Si11Si12_merged_non_normalized.png"
)
...
def create_union_regions(mark):
    ...
    print(
        ...
        f"kept regions with >= {MIN_SAMPLE_COUNT} replicates = {len(filtered)}"
    )
...
# Build the expected non-normalized ChIP/Input BigWig path
# for one cell line, replicate and antibody.
def bw_path(cell, si, mark):
    ...
    return os.path.join(
        BW_DIR,
        f"{sample}_vs_Inp_non_normalized_ratio_bin{BIN}.bw"
    )


# Extract the non-normalized ChIP/Input signal around the union regions with computeMatrix.
...
for mark in MARKS:
    ...
    # Plot the ChIP/Input signal for all union regions and cell lines as a heatmap.
    im = ax_heat.imshow(
        matrix,
        aspect="auto",
...
fig.suptitle(
    "Non-normalized ChIP/Input signal heatmap",
    fontsize=16,
    fontweight="bold",
    y=0.985,
)
